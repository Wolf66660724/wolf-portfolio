...
const INK = '#5b4632';
const ZONES = [
  { id: 'about',   x: 24, y: 36, label: '博客', accent: '#3b82f6', icon: 'book' },
  { id: 'gallery', x: 24, y: 75, label: '留言板', accent: '#22a55a', icon: 'bubble' },
  { id: 'contact', x: 78, y: 21, label: '项目', accent: '#e5484d', icon: 'doc' },
...
function iconSvg(type, cx, cy, color) {
  ...
  if (type === 'doc') {
    ...
    return '<path d="M ' + (cx - 26) + ' ' + (cy - 40) + ' L ' + (cx + 12) + ' ' + (cy - 40) + ' L ' + (cx + 30) + ' ' + (cy - 22) + ' L ' + (cx + 30) + ' ' + (cy + 38) + ' L ' + (cx - 26) + ' ' + (cy + 38) + ' Z" fill="none" stroke="' + c + '" stroke-width="' + s + '" stroke-linejoin="round"/>'
      + '<path d="M ' + (cx + 12) + ' ' + (cy - 40) + ' L ' + (cx + 12) + ' ' + (cy - 22) + ' L ' + (cx + 30) + ' ' + (cy - 22) + '" fill="none" stroke="' + c + '" stroke-width="' + s + '"/>'
      + '<path d="M ' + (cx - 16) + ' ' + (cy - 12) + ' L ' + (cx + 20) + ' ' + (cy - 12) + ' M ' + (cx - 16) + ' ' + (cy + 2) + ' L ' + (cx + 20) + ' ' + (cy + 2) + ' M ' + (cx - 16) + ' ' + (cy + 16) + ' L ' + (cx + 8) + ' ' + (cy + 16) + '" stroke="' + c + '" stroke-width="' + s + '"/>';
  }
  if (type === 'chain') {
...
await render('images/map-v8.webp', buildMap(null));
for (const z of ZONES) {
  await render('images/map-v8-' + z.id + '.webp', buildMap(z.id), { tint: '#f2ecdf' });
}
console.log('MAP V8 DONE');
