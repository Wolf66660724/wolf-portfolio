...
const ZONES = [
  { id: 'about',   x: 25, y: 37, label: '自我介绍', accent: '#e5484d', icon: 'person' },
  { id: 'gallery', x: 25, y: 74, label: '博客项目', accent: '#3b82f6', icon: 'pencil' },
  { id: 'contact', x: 77, y: 22, label: '留言板', accent: '#22a55a', icon: 'bubble' },
  { id: 'studio',  x: 72, y: 61, label: '友链', accent: '#8b5cf6', icon: 'chain' },
];
...
function buildMap(highlight) {
  ...
  // dashed roads
  h += '<path d="M ' + towerX + ' ' + towerY + ' L 400 430" fill="none" stroke="#b7b0a4" stroke-width="5" stroke-dasharray="12 14" opacity="0.7"/>';
  h += '<path d="M ' + towerX + ' ' + towerY + ' L 400 950" fill="none" stroke="#b7b0a4" stroke-width="5" stroke-dasharray="12 14" opacity="0.7"/>';
  h += '<path d="M ' + towerX + ' ' + towerY + ' L 1000 300" fill="none" stroke="#b7b0a4" stroke-width="5" stroke-dasharray="12 14" opacity="0.7"/>';
  h += '<path d="M ' + towerX + ' ' + towerY + ' L 950 800" fill="none" stroke="#b7b0a4" stroke-width="5" stroke-dasharray="12 14" opacity="0.7"/>';
  h += '<path d="M ' + towerX + ' ' + towerY + ' L 676 1240" fill="none" stroke="#b7b0a4" stroke-width="5" stroke-dasharray="12 14" opacity="0.7"/>';

  // tower (warm dark charcoal, solid)
  const tc = highlight === 'tower' ? '#667eea' : '#4a4238';
  h += '<g fill="' + tc + '">'
  ...
  // zones: soft badge + colored icon + colored label
  for (const z of ZONES) {
    const cx = W * z.x / 100, cy = H * z.y / 100;
    const isHi = highlight === z.id;
    // soft white badge behind icon
    h += '<circle cx="' + cx + '" cy="' + (cy - 2) + '" r="' + (isHi ? 66 : 54) + '" fill="#ffffff" opacity="' + (isHi ? 0.85 : 0.6) + '"/>';
    // hover halo
    if (isHi) {
    ...
    h += '<text x="' + cx + '" y="' + (cy + 84) + '" font-family="Hupo" font-size="54" fill="' + z.accent + '" text-anchor="middle">' + z.label + '</text>';
  }

  h += '<circle cx="676" cy="1250" r="16" fill="none" stroke="#4a4238" stroke-width="5" stroke-dasharray="5 6"/>';
  h += '</svg>';
  return h;
}

await render('images/map-v6.webp', buildMap(null));
for (const z of ZONES) {
  await render('images/map-v6-' + z.id + '.webp', buildMap(z.id), { tint: '#f1ece3' });
}
console.log('MAP V6 DONE');
