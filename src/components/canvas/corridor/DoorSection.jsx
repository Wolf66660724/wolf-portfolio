...
const DOOR_LOOK_ANGLE = Math.PI / 2; // face the door straight-on (90°) so the door & interior are not skewed
...
const DOOR_TEXTURES = {
    '博客': '/textures/corridor/doors/drzwiabout.webp',
    '留言板': '/textures/corridor/doors/drzwikontakt.webp',
    '项目': '/textures/corridor/doors/drzwiprojekty.webp',
    '友链': '/textures/corridor/doors/drzwisocial.webp',
};

// Painted (colored) variants for brush-stroke reveal on hover
const DOOR_PAINTED_TEXTURES = {
    '博客': '/textures/corridor/doors/drzwiabout_painted.webp',
    '留言板': '/textures/corridor/doors/drzwikontakt_painted.webp',
    '项目': '/textures/corridor/doors/drzwiprojekty_painted.webp',
    '友链': '/textures/corridor/doors/drzwisocial_painted.webp',
};
...
const DoorSection = ({
    ...
    const doorTiltRef = useRef(); // Subtle door-only tilt toward camera
    ...
    const doorId = useMemo(() => {
        ...
        if (label === '博客') return 'about';
        if (label === '留言板') return 'gallery';
        if (label === '项目') return 'contact';
        if (label === '友链') return 'studio';
        return null;
    }, [label, roomId]);
    ...
    const doorTexturePath = DOOR_TEXTURES[label] || DOOR_TEXTURES['项目'];
    ...
    const doorPaintedTexturePath = DOOR_PAINTED_TEXTURES[label] || DOOR_PAINTED_TEXTURES['项目'];
    ...
    const doorRatio = label === '友链' ? 0.388 : 0.376;
    ...
    useFrame(() => {
        ...
        // === WALL FIXED + DOOR-ONLY TILT ===
        if (!groupRef.current) return;

        const baseDir = side === 'left' ? 1 : -1;
        const tiltDir = side === 'left' ? -1 : 1;

        // The wall NEVER rotates — it stays at its base orientation (walk, open, close).
        const baseAngle = (BASE_ROTATION * baseDir) + (BASE_TILT * tiltDir);
        groupRef.current.rotation.y = baseAngle;

        const absSinBase = Math.abs(Math.sin(baseAngle));
        let baseScale = 1.0;
        if (absSinBase > 0.1) {
            baseScale = (DOOR_Z_SPAN - 0.01) / (WALL_LENGTH * absSinBase);
        }
        groupRef.current.scale.set(THREE.MathUtils.clamp(baseScale, 0.8, 1.1), 1, 1);

        // DOOR ONLY: subtle proximity tilt toward the camera (wall stays put)
        const distance = Math.abs(camera.position.z - position[2]);
        isNearRef.current = distance < 8;

        const DOOR_MAX_TILT = 0.06; // ~3.4 degrees, subtle
        let doorTargetTilt = BASE_TILT;
        if (distance < TILT_START && distance > TILT_PEAK) {
            const t = (TILT_START - distance) / (TILT_START - TILT_PEAK);
            const easedT = t * (2 - t); // easeOutQuad
            doorTargetTilt = BASE_TILT + (DOOR_MAX_TILT - BASE_TILT) * easedT;
        } else if (distance <= TILT_PEAK) {
            doorTargetTilt = DOOR_MAX_TILT;
        }

        currentTilt.current = THREE.MathUtils.lerp(currentTilt.current, doorTargetTilt, 0.08);

        if (doorTiltRef.current) {
            doorTiltRef.current.rotation.y = currentTilt.current * tiltDir;
        }
    });
    ...
    const handleClick = useCallback((e) => {
        ...
        // Reset the door's subtle tilt so the door is flush/centered while aligning & opening
        if (doorTiltRef.current) doorTiltRef.current.rotation.y = 0;
        ...
        const alignDuration = useFastMode ? 0.01 : 0.25;

        // Door center in the wall group's local space. The door panel lives inside
        // the group at [wallOffsetX, -0.4, 0] plus its own offsets, so the true
        // door center is (wallOffsetX, -0.6, 0.015). Convert to world so the
        // camera aligns to the ACTUAL door opening (dead-center).
        const doorWorldPos = new THREE.Vector3(wallOffsetX, -0.6, 0.015);
        groupRef.current.localToWorld(doorWorldPos);

        // Camera moves DIRECTLY in front of the door center (on its normal line),
        // so the door opening is dead-center and the flight goes straight through.
        const CAMERA_DOOR_DIST = 3.4; // far enough to frame the whole door AND the interior behind it
        const cameraTargetZ = doorWorldPos.z;
        const cameraTargetX = side === 'left'
            ? doorWorldPos.x + CAMERA_DOOR_DIST
            : doorWorldPos.x - CAMERA_DOOR_DIST;
    ...
    const openDoor = useCallback((fastMode = false) => {
        ...
        const doorDuration = fastMode ? 0.01 : 0.3;
        const flyDuration = fastMode ? 0.01 : 0.7;
        ...
        gsap.to(doorRef.current.rotation, {
            ...
            onComplete: () => {
                ...
                // Fly STRAIGHT through the door along its entry normal so the room
                // interior stays dead-center even with the slightly angled approach.
                const flyDistance = enterDistance; // Fly through short vestibule (3) + into room

                const entryDir = side === 'left' ? -1 : 1;
                const targetX = camera.position.x + entryDir * flyDistance;
        ...
        <group position={[pivotX, position[1], position[2]]}>
            ...
            <group ref={groupRef}>
                ...
                <group position={[wallOffsetX, -0.4, 0]}>
                    ...
                    <group ref={doorTiltRef} position={[doorPivotX, 0, 0.01]}>
                        <group ref={doorRef}>
                        ...
                        <mesh
                            ...
                            scale={[(side === 'right' && label !== '友链') ? -1 : 1, 1, 1]}
                        >
                            <planeGeometry args={[doorWidth, doorHeight]} />
                            <meshBasicMaterial color="#e0e0e0"
