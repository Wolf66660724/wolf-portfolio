...
const ROOMS = [
    { id: 'about', label: '博客', x: 25, y: 37 },      // Paper airplane (left side)
    { id: 'gallery', label: '留言板', x: 25, y: 74 },  // City buildings (bottom left)
    { id: 'contact', label: '项目', x: 77, y: 22 },  // Pier/dock (top right)
    { id: 'studio', label: '友链', x: 72, y: 61 },    // Monitors stack (right side)
];
...
const NavigationUI = () => {
        ...
        if (paintedMapsRefs.about.current) {
            gsap.to(paintedMapsRefs.about.current, {
                clipPath: (hoveredRoom === 'about' || currentRoom === 'about')
                    ? 'polygon(16% 28%, 34% 28%, 34% 47%, 16% 47%)'
                    : 'polygon(16% 28%, 16% 28%, 16% 47%, 16% 47%)',
                duration: 0.5,
                ease: "power2.out"
            });
        }
        ...
        if (paintedMapsRefs.gallery.current) {
            gsap.to(paintedMapsRefs.gallery.current, {
                clipPath: (hoveredRoom === 'gallery' || currentRoom === 'gallery')
                    ? 'polygon(16% 65%, 34% 65%, 34% 84%, 16% 84%)'
                    : 'polygon(16% 65%, 16% 65%, 16% 84%, 16% 84%)',
                duration: 0.5,
                ease: "power2.out"
            });
        }
        ...
        if (paintedMapsRefs.contact.current) {
            gsap.to(paintedMapsRefs.contact.current, {
                clipPath: (hoveredRoom === 'contact' || currentRoom === 'contact')
                    ? 'polygon(68% 13%, 86% 13%, 86% 32%, 68% 32%)'
                    : 'polygon(86% 13%, 86% 13%, 86% 32%, 86% 32%)',
                duration: 0.5,
                ease: "power2.out"
            });
        }
        ...
        if (paintedMapsRefs.studio.current) {
            gsap.to(paintedMapsRefs.studio.current, {
                clipPath: (hoveredRoom === 'studio' || currentRoom === 'studio')
                    ? 'polygon(63% 52%, 81% 52%, 81% 71%, 63% 71%)'
                    : 'polygon(81% 52%, 81% 52%, 81% 71%, 81% 71%)',
                duration: 0.5,
                ease: "power2.out"
            });
        }
    ...
    // M key — toggle the map panel
    useEffect(() => {
        const handleMKey = (e) => {
            if (hasEntered && (e.key === 'm' || e.key === 'M')) {
            ...
            {hasEntered && (
                <div className={`nav-controls ${isMenuOpen || isAudioMenuOpen ? 'menu-open' : ''} ${isUIHidden ? 'ui-hidden' : ''}`}>
                    ...
                    >
                        ...
                        <span className="nav-btn-label">地图</span>
                    </button>
                    {/* Audio Toggle Button */}
                    <button
                    ...
                    >
                        ...
                        <span className="nav-btn-label">音量</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="nav-btn-label">成就</span>
                    </button>
                </div>
            )}
                ...
                <div className={`map-panel ${isMenuOpen ? 'open' : ''}`} inert={!isMenuOpen ? true : undefined} ref={mapPanelRef} onKeyDown={handleMapKeyDown} role="dialog" aria-label="Map">
                    ...
                    <div className="map-content-clipped">
                        ...
                        <div className="map-container">
                            ...
                            <img src="/images/map-v8.webp" alt="Portfolio Map" className="map-image" />

                            {/* Painted Map Overlays */}
                            <img ref={paintedMapsRefs.about} src="/images/map-v8-about.webp" alt="" className="painted-map-layer" style={{ clipPath: 'polygon(16% 28%, 16% 28%, 16% 47%, 16% 47%)' }} />
                            <img ref={paintedMapsRefs.gallery} src="/images/map-v8-gallery.webp" alt="" className="painted-map-layer" style={{ clipPath: 'polygon(16% 65%, 16% 65%, 16% 84%, 16% 84%)' }} />
                            <img ref={paintedMapsRefs.contact} src="/images/map-v8-contact.webp" alt="" className="painted-map-layer" style={{ clipPath: 'polygon(86% 13%, 86% 13%, 86% 32%, 86% 32%)' }} />
                            <img ref={paintedMapsRefs.studio} src="/images/map-v8-studio.webp" alt="" className="painted-map-layer" style={{ clipPath: 'polygon(81% 52%, 81% 52%, 81% 71%, 81% 71%)' }} />
